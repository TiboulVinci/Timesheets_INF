...
server.use(config.virtualPath, express.static(path.join(__dirname, "..", config.www || "www")));
        ...
        controler = controlerFn({
            ...
            user: new Proxy({}, {
                get(_, k) {
                    if (k == "toJSON") {
                        ...
                        return asyncLocalStorage.getStore().user && asyncLocalStorage.getStore().user[k];
                    }
                },
            ...
            broadcast(canal, msg, oclient) {
                ...
                for (let i = fns.length - 1; i >= 0; i--) {
                    if (fns[i].client == oclient) continue; // avoid self broadcasting
